...
public class bullet : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D hitinfo)
    {
        ...
        BOSS_Turrent turrentUP = hitinfo.GetComponent<BOSS_Turrent>();
        if (turrentUP != null)
        {
            turrentUP.TakeDamage(damage);
        }

        BOSS_TuttentDown turrentDOWN = hitinfo.GetComponent<BOSS_TuttentDown>();
        if (turrentDOWN != null)
        {
            turrentDOWN.TakeDamage(damage);
        }
    }
